...
Win32/Direct2D window (no Edge, no WebView2, no browser dependency), 28 new
tweaks curated from WinUtil's current catalog, a live firmware panel and a
driver auto-update engine.

### Added

- **Native-first window** — double-clicking the exe opens the Direct2D
  liquid-glass dashboard in a real Win32 window: no Edge/WebView2 component, no
  browser, nothing external. The web dashboard is still available via
  `OptimizeKit.exe --app` (WebView2 frame, previous behaviour).
- **28 new tweaks (77 total)** — aligned on WinUtil's current `tweaks.json`:
  Widgets removal, Location tracking, Services-to-Manual (+
  SvcHostSplitThresholdInKB), Store recommended search, Delivery Optimization,
  Consumer features, Restore point, End-task on taskbar right-click, WPBT
  execution block, Razer auto-install block, Notifications, IPv4-preferred /
  IPv6-off / Teredo, Temp files, Disk cleanup + WinSxS trim (DISM /ResetBase),
  Explorer auto-discovery, Hibernation, verbose BSoD, battery %, NumLock on
  boot, Long paths, Sticky-keys prompt, menu delay, Edge debloat (~15 policies),
  Brave debloat (Rewards/Wallet/VPN/P3A), hardware clock to UTC (dual-boot),
  DNS reset to DHCP. All reversible as usual.
- **Firmware & platform panel** (read-only) — live reads of what the firmware
  told Windows: BIOS vendor/version/date, motherboard, boot mode, Secure Boot
  state, TPM presence/version (Win32_Tpm + PnP fallback), VT-x/SVM + running
  hypervisor, modern standby vs S3, HPET, WPBT policy, dynamic tick /
  platform tick / platform clock (bcdedit), kernel dump level, svchost split
  threshold, pending-reboot flag. Endpoint `GET /api/firmware`, CLI
  `OptimizeKit.exe --firmware`. The app never writes firmware.
- **Driver auto-update engine** — real driver ages computed from the
  `Control\Class` driver store (GPU, audio, network, chipset), WAN-Miniport /
  inbox noise filtered out, oldest-first report, problem devices (CM problem
  codes), a Windows Update driver scan trigger (UsoClient, the same orchestrator
  the Settings app uses), `pnputil /scan-devices` rescan and vendor-page
  shortcuts. Endpoints `/api/drvupdate/*`, CLI `OptimizeKit.exe --drvupdate`.
- **New native tabs** — Firmware, Storage (drives with NVMe/SATA bus, largest
  files, one-click cleanups), Startup (Run keys + folders with reversible
  toggles) and a Settings tab (confirm actions, auto-backup, DNS management,
  restore-all) persisted to `config.json`.
- **Web dashboard** — Firmware view merged with the BIOS guide (live cards +
  the 12 firmware recommendation cards), Drivers view with the auto-update
  actions, Settings: DNS→DHCP reset, Delivery Optimization toggle, Store
  suggestions toggle.
- **New icon** — blue/violet gradient bolt on a dark glass tile, generated by
  `tools/make_icon.py` (multi-resolution .ico + PNG set).
- **Quick benchmark on the native dashboard** — the AnTuTu-style score can be
  run from the home tab; the result is remembered for the session.

### Changed

- Launchers and the PowerShell engine bumped to v2.7; build CI passes
  `-lwbemuuid -lcfgmgr32` for the new modules.
- `build.bat` invokes python with `-I` so the Microsoft Store python alias can
  no longer hang the build.
- Gaming profile no longer pulls `ipv6_off` or `disk_cleanup` automatically.

### Fixed

- `/api/firmware` and `/api/drvupdate/report` could stall on server threads:
  COM is now initialized per-thread before every WMI read.
- `store_search_off` restored the ACL on the correct path.
- The v2.6 single-instance probe could leave zombie test instances bound to the
  dashboard port during smoke tests (cleaned up by the CI smoke test).
