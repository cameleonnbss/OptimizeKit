...
const vector<Tweak>& catalog() {
    static const vector<Tweak> c = {
        // ---- Latency / gaming ----
        { "game_mode",        L"Enable Game Mode",                    L"Prioritises the game process for CPU & GPU when a game runs.", false, 1, L"Microsoft / WinUtil" },
        ...
        { "hags_on",          L"Hardware-accelerated GPU scheduling", L"GPU manages its own VRAM queue; lower latency on modern GPUs (needs reboot).", true, 2, L"Microsoft (HAGS)" },
        ...
        { "timer_high",       L"Global timer resolution 0.5ms",       L"Raises the scheduler tick rate for all processes; reduces frame time variance.", true, 3, L"insovs/TimerResolution-Optimization" },
        { "network_gaming",   L"Gaming network stack",                L"TcpAckFrequency=1 + TCPNoDelay (no Nagle), NetworkThrottlingIndex=0xFFFFFFFF, SystemResponsiveness=0 (games MMCSS).", true, 3, L"MS docs / community latency guides" },
        { "mouse_precision",  L"Disable mouse acceleration",          L"Raw 1:1 mouse input (MarkC-style), critical for FPS aim.", false, 2, L"MarkC mouse fix" },
        ...
        { "search_index",     L"Disable search indexing",             L"Removes background disk/CPU indexing churn (set 'classic' on laptops if you search a lot).", true, 2, L"WinUtil" },
        { "sysmain_off",      L"Disable SysMain (Superfetch)",        L"Useless on SSD, consumes RAM & disk. Not recommended on HDD.", true, 2, L"WinUtil / community" },
        ...
        { "power_ultimate",   L"Ultimate Performance power plan",     L"Duplicates + activates the hidden Ultimate Performance scheme.", true, 2, L"Chris Titus Tech WinUtil" },
        ...
        // ---- Privacy / telemetry ----
        { "telemetry_off",    L"Disable telemetry (DiagnosticLog)",   L"AllowTelemetry=0, stops DiagTrack & dmwappushservice.", true, 1, L"WinUtil" },
...
bool apply(const string& id, wstring& err) {
    ...
    if (id == "game_mode") {
        ...
        regSetDword(HKEY_CURRENT_USER, L"Control Panel\\Mouse", L"MouseSpeed", 0);
        regSetDword(HKEY_CURRENT_USER, L"Control Panel\\Mouse", L"MouseThreshold1", 0);
        regSetDword(HKEY_CURRENT_USER, L"Control Panel\\Mouse", L"MouseThreshold2", 0);
    }
...
bool restore(const string& id, wstring& err) {
    if (id == "game_mode")       { regSetDword(HKEY_CURRENT_USER, L"Software\\Microsoft\\GameBar", L"AutoGameModeEnabled", 1); }
        ...
        regSetDword(HKEY_CURRENT_USER, L"Control Panel\\Mouse", L"MouseSpeed", 1);
        regSetDword(HKEY_CURRENT_USER, L"Control Panel\\Mouse", L"MouseThreshold1", 6);
        regSetDword(HKEY_CURRENT_USER, L"Control Panel\\Mouse", L"MouseThreshold2", 10);
    }
